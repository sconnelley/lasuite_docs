...
def get_api_prefix():
    """Extract API prefix from API_VERSION (e.g., 'docs' from 'docs/v1.0')."""
    ...
    def get(self, request):
        ...
        if api_prefix and isinstance(response, HttpResponseRedirect):
            ...
            if "redirect_uri" in query_params:
                ...
                # e.g., /api/v1.0/callback/ -> /api/docs/v1.0/callback/
                if redirect_uri.startswith("/api/"):
                    # Extract version from API_VERSION (e.g., "v1.0" from "docs/v1.0")
                    api_version = getattr(settings, "API_VERSION", "v1.0")
